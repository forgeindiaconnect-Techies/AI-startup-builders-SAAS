              ...
              <div className="bg-gradient-to-br from-purple-50/70 via-white to-emerald-50/50 p-5 rounded-2xl border border-purple-100 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-500">
                        {selectedTx.commissionPaymentMode === 'bank'
                          ? 'Pay via Manual Bank Transfer'
                          : selectedTx.commissionPaymentMode === 'upi_qr'
                          ? `Pay via UPI QR Code (${selectedTx.commissionUpiDetails?.selectedUpiApp === 'gpay' ? 'Google Pay' : selectedTx.commissionUpiDetails?.selectedUpiApp === 'paytm' ? 'Paytm' : selectedTx.commissionUpiDetails?.selectedUpiApp === 'phonepe' ? 'PhonePe' : 'UPI Apps'})`
                          : 'Pay via Manual Bank Transfer or UPI QR Code'}
                      </p>
                    </div>
                  </div>
                  <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 font-extrabold text-[10px] rounded-full border border-emerald-200">
                    {selectedTx.commissionPaymentMode === 'bank' ? 'Manual Transfer Only' : selectedTx.commissionPaymentMode === 'upi_qr' ? 'UPI QR Only' : 'Bank & UPI Ready'}
                  </span>
                </div>

                <div className={`grid gap-4 pt-1 ${selectedTx.commissionPaymentMode === 'both' || !selectedTx.commissionPaymentMode ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
                  {/* Manual Bank Details - Show ONLY if mode is 'bank' or 'both' */}
                  {(selectedTx.commissionPaymentMode === 'bank' || selectedTx.commissionPaymentMode === 'both' || !selectedTx.commissionPaymentMode) && (
                  ...
                  {/* Paytm / GPay / PhonePe UPI & QR - Show ONLY if mode is 'upi_qr' or 'both' */}
                  {(selectedTx.commissionPaymentMode === 'upi_qr' || selectedTx.commissionPaymentMode === 'both' || !selectedTx.commissionPaymentMode) && (
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-2xs space-y-2">
                      <div className="flex items-center justify-between text-gray-900 font-bold text-xs">
                        ...
                        <div className="flex gap-1">
                          {(selectedTx.commissionUpiDetails?.selectedUpiApp === 'paytm' || selectedTx.commissionUpiDetails?.selectedUpiApp === 'all' || !selectedTx.commissionUpiDetails?.selectedUpiApp) && (
                            <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-[9px] font-extrabold rounded">Paytm</span>
                          )}
                          {(selectedTx.commissionUpiDetails?.selectedUpiApp === 'gpay' || selectedTx.commissionUpiDetails?.selectedUpiApp === 'all' || !selectedTx.commissionUpiDetails?.selectedUpiApp) && (
                            <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 text-[9px] font-extrabold rounded">GPay</span>
                          )}
                          {(selectedTx.commissionUpiDetails?.selectedUpiApp === 'phonepe' || selectedTx.commissionUpiDetails?.selectedUpiApp === 'all' || !selectedTx.commissionUpiDetails?.selectedUpiApp) && (
                      ...
                      <div className="pt-1 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-500">
                        <span>✓ {selectedTx.commissionUpiDetails?.selectedUpiApp === 'gpay' ? 'Google Pay' : selectedTx.commissionUpiDetails?.selectedUpiApp === 'paytm' ? 'Paytm' : selectedTx.commissionUpiDetails?.selectedUpiApp === 'phonepe' ? 'PhonePe' : 'Paytm • GPay • PhonePe'} Supported</span>
