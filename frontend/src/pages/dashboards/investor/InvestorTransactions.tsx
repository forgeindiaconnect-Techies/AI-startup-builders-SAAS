        ...
        <div className="fixed inset-0 z-[120] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl relative my-8 animate-in zoom-in-95 duration-200 flex flex-col text-left">
            ...
            <div className="p-6 sm:p-8 py-4 overflow-y-auto space-y-6 max-h-[60vh]">
              ...
              {(() => {
                const mode = selectedTx.commissionPaymentMode || 'bank';
                const showBank = mode === 'bank' || mode === 'both';
                const showUpi = mode === 'upi_qr' || mode === 'both';
                  ...
                  <div className="bg-gradient-to-br from-purple-50/70 via-white to-emerald-50/50 p-5 rounded-2xl border border-purple-100 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 font-extrabold text-[10px] rounded-full border border-emerald-200">
                        {mode === 'bank' ? 'Manual Transfer Only' : mode === 'upi_qr' ? 'UPI QR Only' : 'Bank & UPI Ready'}
                      </span>
                    </div>

                    <div className={`grid gap-4 pt-1 ${mode === 'both' ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
